...
from train_v4_small import load_batch


def main():
    model = tf.keras.models.load_model('/data/training/v4_small/model_roboto')
    root_root = '/data/ground_truth/roboto'
    results_root_root = '/data/results/v4_smaller/roboto'
    ...
    for weight_name, weight in font_weights.items():
        root = os.path.join(root_root, weight_name)
        results_root = os.path.join(results_root_root, weight_name)
        ...
        glyph_metadata = pd.read_csv(os.path.join(root_root, 'glyphs.csv'))
        ...
        for subdir_idx, subdir in tqdm(enumerate(subdirs), total=len(subdirs), desc=weight_name):
            curr_errors = []
            ...
            for i, (_, glyph_num, _, _) in enumerate(curr_batch):
