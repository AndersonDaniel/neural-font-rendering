...
def main():
    models = [
        tf.keras.models.load_model('/data/training/v9_real_small/cap_a/model_arsenica'),
        tf.keras.models.load_model('/data/training/v9_real_small/cap_a/checkpoints/model_arsenica_1'),
        tf.keras.models.load_model('/data/training/v9_real_small/cap_a/checkpoints/model_arsenica_2'),
        tf.keras.models.load_model('/data/training/v9_real_small/cap_a/checkpoints/model_arsenica_3'),
        tf.keras.models.load_model('/data/training/v9_real_small/cap_a/checkpoints/model_arsenica_4')
    ]
    ...
    results_root = '/data/results/v9_real_small/cap_a/arsenica'
    ...
    for e_idx, (e, e_subdirs) in enumerate(subdirs.items()):
        for subdir_idx, subdir in tqdm(enumerate(e_subdirs), total=len(e_subdirs)):
            ...
            for i, sample in enumerate(curr_batch):
                ...
                Y_preds = [
                    model([curr_X_glyph, curr_X_size, curr_X_pos, curr_X_extra])[1].numpy()
                    for model in models
                ]

                Y_pred = np.median(np.stack(Y_preds, axis=0), axis=0)
