...
def make_model(n_glyphs, n_fonts):
    ...
    font_embedding_layer = tf.keras.layers.Embedding(n_fonts, 1420, input_length=1)
    ...
    embedding_extra_modification_model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(300, input_shape=(font_embedding.shape[1] + x_extra.shape[1],),
                              activation='relu'),
        tf.keras.layers.Dense(300, activation='relu'),
        tf.keras.layers.Dense(300, activation='relu'),
        tf.keras.layers.Dense(300, activation='relu'),
        tf.keras.layers.Dense(300, activation='relu'),
        tf.keras.layers.Dense(font_embedding.shape[1], activation='tanh')
    ])
...
def run_experiment(subdirs, glyph_nums, glyph_metadata, bitmap_size, n_sizes,
    ...
    if model is None:
        model = make_model(glyph_metadata.shape[0], len(subdirs))
...
def main():
    ...
    with open(f'/data/training/v10_real_small/{RES_NAME}/loss_hist.json', 'w') as f:
        json.dump(hist, f)

    with open(f'/data/training/v10_real_small/{RES_NAME}/loss_max_hist.json', 'w') as f:
        json.dump(max_hist, f)

    model.save(f'/data/training/v10_real_small/{RES_NAME}/model')
    ...
    # imageio.mimsave(f'/data/training/v10_real_small/{RES_NAME}/training_history.gif', viz_hist_pil, duration=.2)
