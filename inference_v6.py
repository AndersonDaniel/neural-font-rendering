...
from train_v6_small import load_batch, load_glyph_nums


def main():
    # model = tf.keras.models.load_model('/data/training/v6_small/model_times_new_roman')
    model = tf.keras.models.load_model('/data/training/v6_real_small/w/model_times_new_roman')
    root = '/data/ground_truth/times_new_roman'
    results_root = '/data/results/v6_real_small/w/times_new_roman'
    ...
    for subdir_idx, subdir in tqdm(enumerate(subdirs), total=len(subdirs)):
        ...
        for i, sample in enumerate(curr_batch):
            ...
            Y_pred = model([curr_X_glyph, curr_X_size, curr_X_pos])[:, 0].numpy()
