...
from train_v8_small import load_batch, load_glyph_nums, INTENSITY_CATEGORIES


def main():
    model = tf.keras.models.load_model('/data/training/v8_real_small/abcxyz/model_times_new_roman')
    root = '/data/ground_truth/times_new_roman'
    results_root = '/data/results/v8_real_small/abcxyz/times_new_roman'
    ...
    for subdir_idx, subdir in tqdm(enumerate(subdirs), total=len(subdirs)):
        ...
        for i, sample in enumerate(curr_batch):
            ...
            Y_pred = model([curr_X_glyph, curr_X_pos]).numpy().argmax(axis=1).astype(float) / INTENSITY_CATEGORIES
