...
def run_experiment(subdirs, glyph_nums, glyph_metadata, bitmap_size, n_sizes,
    ...
    if model is None:
        model, inner_model = make_model(glyph_metadata.shape[0], len(subdirs))
    ...
    for epoch in range(1, n_epochs):
        ...
        for curr_batch_indices in batch_pb:
            ...
            with tf.GradientTape() as tape:
                pred_cat, pred, embedding  = model([curr_X_glyph, curr_X_size,
                                                    curr_X_pos, curr_X_font, curr_X_extra],
                                                   training=True)
                                                
                print(discriminator(tf.reshape(pred, (-1, actual_size, actual_size, 1))).shape)
                exit()
...
def main():
    ...
    hist, model = run_experiment(subdirs, glyph_nums, glyph_metadata, bitmap_size, n_sizes,
                                 base_names, modifier_names, RES_NAME,
                                 n_epochs=500)

    

    with open(f'/data/training/v11_real_small/{RES_NAME}/loss_hist.json', 'w') as f:
        json.dump(hist, f)
