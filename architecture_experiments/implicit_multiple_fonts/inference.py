...
def main():
    ...
    results_root = f'/home/ubuntu/data/results/implicit_multiple_fonts/exp2/{EXP_NAME}/results'
        ...
        for e_idx, (e, e_subdirs) in enumerate(font_subdirs.items()):
            ...
            for subdir_idx, subdir in tqdm(enumerate(e_subdirs), total=len(e_subdirs)):
                ...
                for i, sample in enumerate(curr_batch):
                    ...
                    Y_pred_cat, Y_pred, _ = model([curr_X_glyph, curr_X_size, curr_X_pos, curr_X_font, extra_embedding],
                                                  training=True)
                    Y_pred = Y_pred.numpy()
