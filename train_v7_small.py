...
def run_experiment(subdirs, glyph_nums, glyph_metadata, bitmap_size,
    ...
    for epoch in range(1, n_epochs):
        ...
        for curr_batch_idx in batch_pb:
            ...
            curr_X_glyph, curr_X_size, curr_X_pos, curr_Y, _ = load_batch([
                all_combinations[i]
            ...
            with tf.GradientTape() as tape:
                pred = model([curr_X_glyph, curr_X_size, curr_X_pos], training=True)
...
def full_evaluation_report(all_combinations, n_sizes, glyph_metadata,
    ...
    for sample in sample_pb:
        curr_X_glyph, curr_X_size, curr_X_pos, curr_Y, _ = load_batch([sample], n_sizes, glyph_metadata,
                                    bitmap_size, base_names, modifier_names)
        pred_Y = model([curr_X_glyph, curr_X_size, curr_X_pos])
...
def load_batch(batch, n_sizes, glyph_metadata, bitmap_size, base_names, modifier_names):
    ...
    return X_glyph, X_size, X_pos, categorical_Y, Y
...
def main():
    ...
    os.makedirs('/data/training/v7_real_small/abcxyz', exist_ok=True)
    os.makedirs('/data/results/v7_real_small/abcxyz', exist_ok=True)
    with open('/data/training/v7_real_small/abcxyz/loss_hist_times_new_roman.json', 'w') as f:
        json.dump(hist, f)

    with open('/data/training/v7_real_small/abcxyz/loss_max_hist_times_new_roman.json', 'w') as f:
        json.dump(max_hist, f)

    model.save('/data/training/v7_real_small/abcxyz/model_times_new_roman')
