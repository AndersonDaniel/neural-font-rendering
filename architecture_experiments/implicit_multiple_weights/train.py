...
def run_experiment(subdirs, glyph_nums, glyph_metadata, bitmap_size, n_sizes,
    ...
    if model is None:
        weight_embedding_model, model = make_model(glyph_metadata.shape[0], len(subdirs))
    ...
    for epoch in range(1, n_epochs):
        ...
        for curr_batch_idx in batch_pb:
            ...
            with tf.GradientTape() as tape:
                ...
                pred_cat, pred, embedding  = model([curr_X_glyph, curr_X_size, curr_X_pos, curr_X_font, weight_embedding],
                                                   training=True)
            ...
            grads = tape.gradient(loss_value, weight_embedding_model.trainable_weights + model.trainable_weights)
            optimizer.apply_gradients(zip(grads, weight_embedding_model.trainable_weights + model.trainable_weights))
    ...
    return loss_history, max_loss_history, weight_embedding_model, model, visualization_history
...
def main():
    ...
    # for glyph_idx in map(ord, 'agmt'):
        ...
        hist, max_hist, weight_embedding_model, model, visualization_history = run_experiment(subdirs, glyph_nums, glyph_metadata, bitmap_size, n_sizes,
                                                                    base_names, modifier_names,
                                                                    eps=0, loss_patience=20, n_epochs=500)


        results_root = os.path.join('/home/ubuntu/data/results/implicit_multiple_weights', 'roboto', EXP_NAME)
        # results_root = os.path.join('/home/ubuntu/data/results/implicit_multiple_weights', 'arsenica', EXP_NAME)
        ...
        weight_embedding_model.save(os.path.join(results_root, 'weight_embedding_model'))
