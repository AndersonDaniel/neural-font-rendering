...
def main():
    ...
    for FONT in ['times_new_roman']:
        ...
        #                   *list(map(ord, list(',./?><!@#$%^&*()-=_+')))]:
            ...
            model = tf.keras.models.load_model(f'/home/ubuntu/data/results/implicit_no_memorization/{FONT}/{EXP_NAME}/model')

            root = f'/home/ubuntu/data/ground_truth/{FONT}'
            results_root = f'/home/ubuntu/data/results/implicit_no_memorization/{FONT}/{EXP_NAME}/results'
            ...
            for subdir_idx, subdir in tqdm(enumerate(subdirs), total=len(subdirs)):
                ...
                for i, sample in enumerate(curr_batch):
                    ...
                    pred = model([curr_X_glyph, curr_X_pos])
