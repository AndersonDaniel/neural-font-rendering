...
def main(FONT_EMBEDDING_DIM):
    model = tf.keras.models.load_model('/data/training/v11_real_small/cap_a/inner_model')

    results_root = '/data/results/v11_real_small/cap_a_random_fonts/'
    ...
    N_FONTS = 5
    EXTRAS = [i / 7 for i in range(1, 8)]

    for font_idx in range(N_FONTS):
        font_embedding = tf.random.normal((FONT_EMBEDDING_DIM,)),
        for e_idx, e in enumerate(EXTRAS):
            for size_idx in tqdm(range(MAX_BMP_SIZE - MIN_BMP_SIZE + 1)):
                curr_size = size_idx + MIN_BMP_SIZE
                curr_comparison_results_path = os.path.join(comparison_results_path, str(font_idx), str(e_idx), f'{curr_size}_foo')
                ...
                X_extra = e * tf.ones((X_size.shape[0], 1))
